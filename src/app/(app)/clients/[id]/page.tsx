        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex gap-2">
              ...
              {client.isPublicSector && <Badge variant="warning">Secteur public — Facture via Chorus Pro</Badge>}
              {client.deletedAt && <Badge variant="destructive">Archivé</Badge>}
            </div>
          </div>

          {canWrite && (
            <div className="flex gap-3">
              <Button render={<Link href={`/clients/${client.id}/modifier`} />} nativeButton={false}>Modifier</Button>
            </div>
          )}
        </div>
      </PageHero>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 'var(--space-7)' }}>
            ...
            <Card>
              <CardContent>
                <form action={client.deletedAt ? restoreClientAction : archiveClientAction}>
                  ...
                  <Button type="submit" variant={client.deletedAt ? 'secondary' : 'destructive'} size="sm">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            {canWrite && (
              <Button render={<Link href={`/clients/${client.id}/contacts/nouveau`} />} nativeButton={false} variant="secondary" size="sm">
                + Ajouter
              </Button>
            )}
          </CardHeader>
