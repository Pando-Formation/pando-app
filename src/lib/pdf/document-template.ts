...
/** The inner content of one document — everything a page needs except the outer <html>/<head>/<style> wrapper, so several can be rendered as separate pages of one PDF (see renderMultiDocumentHtml). */
function renderDocumentBody(input: DocumentRenderInput): string {
...
<body>
  <div class="doc-page">${renderDocumentBody(input)}</div>
</body>
</html>`
}

/**
 * One PDF, several independent documents inside it — one page (or more) per
 * entry, each with its own title/reference/recipient, separated by a real
 * page break. Used for per-participant bundles (e.g. one attestation per
 * participant of a contractualisation) instead of a single shared roster.
 */
export function renderMultiDocumentHtml(bundleTitle: string, sections: DocumentRenderInput[]): string {
  const pages = sections.map((s) => `<div class="doc-page">${renderDocumentBody(s)}</div>`).join('')
  return `<!doctype html>
<html lang="fr">
<head>
<meta charset="utf-8" />
